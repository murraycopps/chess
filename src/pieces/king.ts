...
export default class King extends Piece {
  ...
  validMoves(squares: PieceType[][], checks: boolean = true) {
    ...
    const moves = checks
      ...
      // filter out moves enemy peices can move to
      .filter((move) => {
        const array: PieceType[][] = [...squares].map((a) => [...a]);
        let valid = true;
        ...
        array.forEach((a) => {
          a.forEach((b) => {
            if (typeof b !== "string" && b.color !== this.color) {
              const captureMoves = b.capture(array, false);
              const validMoves = b.validMoves(array, false);
              captureMoves.forEach((moveC) => {
                if (move.x === moveC.x && move.y === moveC.y) {
                  valid = false;
                }
              });
              validMoves.forEach((moveV) => {
                if (move.x === moveV.x && move.y === moveV.y) {
                  valid = false;
                }
              });
            }
  ...
  capture(squares: PieceType[][]) {
    ...
    return validMoves.filter((move) => {
      ...
      // filter out moves that are defended by other pieces
      .filter((move) => {
        const array: PieceType[][] = [...squares].map((a) => [...a]);
        let defended = false;
        // check for check from the enemy
        array.forEach((a) => {
          a.forEach((b) => {
            if (typeof b !== "string" && b.color !== this.color) {
              const defendedMoves = b.defended(array, false);
              defendedMoves.forEach((moveD) => {
